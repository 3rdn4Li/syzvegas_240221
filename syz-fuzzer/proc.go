...
func (proc *Proc) ProcessItem(item interface{}) (int, interface{}) {
	if item != nil {
		...
		case *WorkCandidate:
			{
				proc.fuzzer.writeLog("# %s\n", "WorkCandidate")
				_, res := proc.execute(proc.execOpts, item.p, item.flags, StatCandidate, false)
				return 0, res
			}
		case *WorkSmash:
			{
...
func (proc *Proc) triageInput(item *WorkTriage) TriageResult {
	...
	for i := 0; i < signalRuns; i++ {
		proc.resetAccState()
		info, _time := proc.executeRaw(proc.execOptsCover, item.p, StatTriage) //executeRaw also returns the time for execution
		...
		item.p, item.call = prog.Minimize(item.p, item.call, false,
			func(p1 *prog.Prog, call1 int) bool { 
				...
				for i := 0; i < minimizeAttempts; i++ {
					...
					var r ExecResult
					info,r = proc.execute(proc.execOpts, p1, ProgNormal,
						StatMinimize, i == 0)
...
func (proc *Proc) executeHintSeed(p *prog.Prog, call int) {
	...
	info, _ := proc.execute(proc.execOptsComps, p, ProgNormal, StatSeed,true)
	if info == nil {
		return
	}
...
func (proc *Proc) execute(execOpts *ipc.ExecOpts, p *prog.Prog, flags ProgTypes, stat Stat, resetState bool) (*ipc.ProgInfo, ExecResult) {
	ret := ExecResult{
		gainRaw: 0.0,
		time:    0,
		calls:   0,
		pidx:    -1, //program index I guess
	}
	if resetState {
		proc.resetAccState()
	}
...
//return what proc.execute returns
func (proc *Proc) executeAndCollide(execOpts *ipc.ExecOpts, p *prog.Prog, flags ProgTypes, stat Stat) (*ipc.ProgInfo, ExecResult)  {
	info, ret:=proc.execute(execOpts, p, flags, stat, true)
	...
	// Lets's don't consider the cost of collide currently
